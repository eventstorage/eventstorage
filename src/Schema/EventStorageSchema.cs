...
public abstract class EventStorageSchema(string schema) : IEventStorageSchema
{
    ...
    public virtual string LoadEventSourceCommand(string sourceTId) => @$"SELECT {Sequence},
        {LongSourceId}, {GuidSourceId}, {Type}, {Data} FROM {schema}.EventSources WHERE
        {sourceTId} = @sourceId ORDER BY Version ASC";
    ...
    public virtual string InsertCheckpointCommand => @$"INSERT INTO {Schema}.Checkpoints
        (Subscription, Type, Sequence) VALUES (@subscription, @type, @sequence)";
